...
from ..deps import ClientDep, ConnDep, RegistryDep, SettingsDep
...
class RunRuleRequest(BaseModel):
    ...
    target_graph: str | None = Field(
        ...
        description=(
            ...
            "output somewhere inspectable without writing to what it would "
            "normally touch."
        ),
    )
    dry_run: bool = Field(
...
async def get_named_rule(
    rule_id: str, conn: ConnDep, client: ClientDep, cache: RegistryDep
) -> dict:
    ...
    if rule is None:
        raise HTTPException(
            ...
            detail={
                ...
                "id": rule_id,
                "available": [r.id for r in result.rules],
            },
        )
    return rule.detail()
...
async def run_named_rule(
    ...
    if body.dry_run:
        ...
        try:
            sparql = bind_rule(rule, body.params)
    ...
    return await _run(
        conn, client, rule, body.params, body.write_mode, body.timeout, body.target_graph
    )


@router.post("/named-rules/run")
async def run_all_named_rules(
    body: RunAllRequest, conn: ConnDep, client: ClientDep, cache: RegistryDep
) -> dict:
    """Fire every Active rule once, in declared order.
    ...
    """
    result = await cache.get(client, conn, kind=KIND, loader=load_named_rules)
    active = [r for r in result.rules if r.runnable]
    ...
    for rule in active:
        try:
            runs.append(
                await _run(conn, client, rule, body.params, None, body.timeout)
            )
