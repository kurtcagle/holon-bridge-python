...
#: "Bearer " prefix) and says whether it is valid. Kept this narrow so a new
#: credential kind is one function, not a BearerGate subclass.
...
class BearerGate:
    ...
    def _authorised(self, scope: Scope) -> bool:
        for name, value in scope.get("headers", []):
            ...
            return any(check(credential) for check in self._checkers)
        return False
...
def build_app(mcp, transport: str):  # noqa: ANN001 - FastMCP instance
    ...
    if oauth_config is not None:
        checkers.append(
            lambda presented: github_oauth.verify_session_token(oauth_config, presented)
            is not None
        )
